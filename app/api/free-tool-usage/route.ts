...
export async function POST(request: NextRequest) {
  try {
    ...
    // Log the usage
    const { error: insertError } = await supabase.from('free_tool_usage').insert({
      tool_name: toolName,
    ...
    let remaining = null
    if (!user) {
      const { data: rateLimit } = await supabase.rpc('check_free_tool_rate_limit', {
        p_tool_name: toolName,
        p_ip_address: ip,
        p_fingerprint: fingerprint || ip,
      })
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check rate limit
    const { data: rateLimit, error } = await supabase.rpc('check_free_tool_rate_limit', {
      p_tool_name: toolName,
      p_ip_address: ip,
      p_fingerprint: fingerprint || ip,
    })
