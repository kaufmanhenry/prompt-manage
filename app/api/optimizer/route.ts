...
function analyzePromptStructure(prompt: string): PromptAnalysis {
  ...
  if (prompt.length < 50) {
    ...
    analysis.improvements.push(
      'Prompt is very long - consider breaking into smaller, focused parts',
    )
  ...
  const hasFormatting =
    /\b(format|structure|style|tone|length|bullet|list|paragraph|section)\b/i.test(prompt)
  if (hasFormatting) {
    analysis.score += 10
  ...
  analysis.clarity = Math.min(
    100,
    analysis.clarity + contextMatches.length * 15 - foundVagueWords.length * 5,
  )
...
Be constructive and practical in your suggestions.`,
        ...
        const optimizedMatch = aiResponse.match(
          /(?:optimized prompt|improved version|better prompt):\s*(.+)/i,
        )
        if (optimizedMatch) {
    ...
    const response: PromptAnalysis = {
      ...
      suggestions: [...(structureAnalysis.improvements || []), ...aiSuggestions].slice(0, 8), // Limit total suggestions
