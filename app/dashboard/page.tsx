...
import { subscribeToPromptsChanges } from '@/lib/realtime/prompts';
...
export default function DashboardPage() {
  ...
  const { data: prompts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supa.from('prompts').select('*').order('updated_at', { ascending: false });
      if (currentWorkspace.type === 'org') {
        query = query.eq('org_id', currentWorkspace.orgId);
      } else if (session?.user?.id) {
        query = query.eq('user_id', session.user.id).is('org_id', null);
      }
  ...
  // Realtime updates for prompts scoped to workspace
  useEffect(() => {
    if (!session?.user?.id) return;
    const unsub = subscribeToPromptsChanges(
      currentWorkspace.type === 'org'
        ? { type: 'org', orgId: currentWorkspace.orgId }
      ...
      () => {
        ...
        queryClient.invalidateQueries({
          queryKey: [
            'prompts',
            currentWorkspace.type === 'org' ? currentWorkspace.orgId : 'personal',
          ],
        });
      }
    );
    return () => unsub();
  }, [currentWorkspace, queryClient, session?.user?.id]);
