...
import { Copy, Download, CheckCircle2, AlertCircle, Sparkles, Wand2, Loader2 } from 'lucide-react'
...
function charBadge(text: string, limit: number) {
  ...
  return (
    <span className={cn('px-1.5 py-0.5 rounded text-[10px]', isOk ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700')}>
...
function toCsv(rows: GeneratedAd[]): string {
  ...
  for (const r of rows) {
    const cells = [r.variant, r.primaryText, r.headline, r.description, r.cta].map((v) =>
      '"' + String(v).replace(/"/g, '""') + '"'
    )
...
export default function MetaAdGenerator() {
  ...
  const [benefit, setBenefit] = useState('Ship high-performing ads faster with consistent, on-brand copy')
  ...
  function buildPrimaryText(i: number) {
    const pieces: string[] = []
    pieces.push(offer)
    ...
    if (angles.includes('Problem/Solution')) pieces.push('Stop ad fatigue: keep variants fresh without the chaos')
    const base = pieces.join('. ')
    const toned = tone === 'Urgent' ? base + ' Now.' : base
    return clampToLimit(toned, RECOMMENDED_LIMITS.primaryText)
  }

  function buildHeadline(i: number) {
    const patterns = [
  ...
  const allValid = useMemo(() =>
    rows.length > 0 && rows.every(
      (r) => r.primaryText.length <= RECOMMENDED_LIMITS.primaryText && r.headline.length <= RECOMMENDED_LIMITS.headline && r.description.length <= RECOMMENDED_LIMITS.description
    ),
  [rows])
    ...
    <div className="mx-auto max-w-md rounded-2xl border border-emerald-200/50 dark:border-emerald-900/40 bg-white dark:bg-gray-900 p-5 shadow-sm ring-1 ring-emerald-500/10">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
            Meta Ad Generator <Badge variant="outline" className="ml-1">For marketers</Badge>
          </span>
        </div>
      ...
      <div className="space-y-3">
        <div>
          ...
          <Input value={offer} onChange={(e) => setOffer(e.target.value)} placeholder="Product/offer the ad is about" />
        </div>
        <div>
        ...
        <div>
          ...
          <Textarea value={benefit} onChange={(e) => setBenefit(e.target.value)} className="min-h-[60px]" placeholder="What outcome do they get?" />
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="w-[90px]">
            ...
            <Input type="number" className="h-8" value={variants} min={1} max={10} onChange={(e) => setVariants(Number(e.target.value || 1))} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {ANGLES.map((a) => (
              <Button key={a} size="sm" variant={angles.includes(a) ? 'default' : 'outline'} onClick={() => toggleAngle(a)}>
                {a}
              </Button>
            ))}
          </div>
        ...
        <div className="space-y-2">
                  ...
                  {rows.map((r) => (
                    <TableRow key={r.variant}>
                      ...
                      <TableCell className="text-right">
                        <Button size="sm" variant="ghost" onClick={() => copyText(`${r.primaryText}\n${r.headline}\n${r.description}\n${r.cta}`)}>
          ...
          {rows.length > 0 && (
            <div className={cn('text-xs flex items-center gap-2', allValid ? 'text-emerald-600' : 'text-red-600')}>
              {allValid ? (
                <>
                ...
                <>
                  <AlertCircle className="w-4 h-4" /> Some variants exceed limits—edit or trim before launching
                </>
              )}
            </div>
          )}
