...
function generateRuleBasedAudienceDescription(prompt: PromptContext): EnhancedAudienceDescription {
  const combined =
    `${prompt.name} ${prompt.description || ''} ${(prompt.tags || []).join(' ')} ${prompt.promptText || ''}`.toLowerCase()
 ...
 */
function detectIndustryAndPersonas(
  combined: string,
  name: string,
): { industry: string; personas: PersonaInfo[] } {
...
function detectSkillLevel(
  ...
  const beginnerMatches = SKILL_LEVEL_INDICATORS.beginner.filter(
    (indicator) => combined.includes(indicator) || name.toLowerCase().includes(indicator),
  ).length

  const intermediateMatches = SKILL_LEVEL_INDICATORS.intermediate.filter(
    (indicator) => combined.includes(indicator) || name.toLowerCase().includes(indicator),
  ).length

  const advancedMatches = SKILL_LEVEL_INDICATORS.advanced.filter(
    (indicator) => combined.includes(indicator) || name.toLowerCase().includes(indicator),
  ).length
...
function generateContextualDescription(
  ...
  const templates = [
    // Industry-focused
    (_p: PromptContext, ind: string, skill: string) => {
      if (personas.length === 0)
        return `Essential for ${ind.toLowerCase()} professionals seeking to ${extractAction(prompt.name)} more effectively.`
    ...
    (_p: PromptContext, ind: string, skill: string) => {
      if (personas.length === 0)
        return `Perfect for ${skill} users looking to ${extractAction(prompt.name)} efficiently.`
      const personaList = formatPersonaList(personas.map((p) => p.name))
      return `Tailored for ${skill} ${personaList} who want to ${extractAction(prompt.name)} professionally.`
    },

    // Use-case focused
    (_p: PromptContext, _ind: string, _skill: string) => {
      if (personas.length === 0)
        return `Ideal for professionals working on ${extractUseCase(prompt.name)} projects.`
      const personaList = formatPersonaList(personas.map((p) => p.name))
    ...
    (_p: PromptContext, _ind: string, _skill: string) => {
      if (personas.length === 0)
        return `Created for teams that value efficiency when ${extractAction(prompt.name)}.`
      const personaList = formatPersonaList(personas.map((p) => p.name))
...
function generateSEOMetadata(
  ...
  const intentKeywords = ['create', 'generate', 'build', 'design', 'analyze', 'optimize', 'improve']

  return {
    primaryKeywords: Array.from(new Set(primaryKeywords)),
    longTailKeywords: Array.from(new Set(longTailKeywords)),
    semanticKeywords: Array.from(new Set(semanticKeywords)),
...
function detectFallbackPersonas(combined: string, name: string): PersonaInfo[] {
  ...
  for (const [persona, keywords] of Object.entries(fallbackMap)) {
    const matches = keywords.filter(
      (keyword) => combined.includes(keyword) || name.toLowerCase().includes(keyword),
    )
    if (matches.length > 0) {
      detected.push({
        name: persona,
...
function extractUseCase(name: string): string {
  ...
  if (name.toLowerCase().includes('blog') || name.toLowerCase().includes('article'))
  ...
  if (name.toLowerCase().includes('code') || name.toLowerCase().includes('development'))
    return 'software development'
  if (name.toLowerCase().includes('analysis') || name.toLowerCase().includes('data'))
 ...
 */
export function generateEnhancedCTA(audience: EnhancedAudienceDescription): {
  text: string
  emphasis?: string
} {
  const { industry, skillLevel, personas: _personas } = audience
