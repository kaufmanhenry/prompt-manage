...
function generatePrimaryDescription(
  ...
  const templates: ((
    ...
    (p, n) => {
      if (p.length === 0)
        return `Perfect for anyone looking to ${extractAction(n)} efficiently using AI.`
    ...
    (p, n) => {
      if (p.length === 0)
        return `Ideal for professionals seeking to ${extractAction(n)} faster and more effectively.`
      const personaList = formatPersonaList(p)
    ...
    (p, n) => {
      if (p.length === 0)
        return `Valuable for ${extractAction(n)} across various projects and workflows.`
      const personaList = formatPersonaList(p)
...
function extractBenefit(name: string, combined: string): string {
  if (combined.includes('faster') || combined.includes('quick'))
    return 'work faster without sacrificing quality'
  if (combined.includes('professional') || combined.includes('quality'))
    return 'deliver professional-grade results'
  if (combined.includes('scale') || combined.includes('automate'))
    return 'scale their operations efficiently'
  if (combined.includes('convert') || combined.includes('sales'))
    return 'increase conversions and close more deals'
  if (combined.includes('engage') || combined.includes('audience'))
    return 'engage their audience effectively'

  return `${extractAction(name)} more effectively`
}

function extractOutcome(name: string, combined: string): string {
  if (combined.includes('traffic') || combined.includes('seo'))
    return 'boost organic traffic and search rankings'
  if (combined.includes('revenue') || combined.includes('sales')) return 'drive revenue growth'
  if (combined.includes('engagement') || combined.includes('social'))
    return 'increase engagement and reach'
  if (combined.includes('productivity') || combined.includes('efficient'))
    return 'maximize productivity'
  if (combined.includes('quality') || combined.includes('professional'))
    return 'deliver exceptional quality'
...
function extractTask(name: string, combined: string): string {
  ...
  if (combined.includes('code') || combined.includes('development'))
    return 'development and debugging'
...
function extractQualityGoal(name: string, combined: string): string {
  if (combined.includes('conversion') || combined.includes('persuasive'))
    return 'high-converting, persuasive copy'
  if (combined.includes('seo') || combined.includes('optimization'))
    return 'SEO-optimized, discoverable content'
  if (combined.includes('professional') || combined.includes('polished'))
    return 'professional, polished results'
  if (combined.includes('creative') || combined.includes('original'))
    return 'creative, original content'
  if (combined.includes('accurate') || combined.includes('precise'))
    return 'accurate, detailed outputs'

  return 'high-quality results'
}

function extractUseCase(name: string, combined: string): string {
  if (combined.includes('campaign') || combined.includes('launch'))
    return 'campaigns and product launches'
  ...
  if (combined.includes('email') || combined.includes('newsletter'))
    return 'email marketing and newsletters'
  if (combined.includes('product') || combined.includes('description'))
    return 'product descriptions and listings'
...
export function generateCTA(prompt: PromptContext): { text: string; emphasis?: string } {
  const combined =
    `${prompt.name} ${prompt.description || ''} ${(prompt.tags || []).join(' ')}`.toLowerCase()
